...
MatchRule_Sh::MatchRule_Sh(void)
	:m_scl(keywords::channel = "MatchRule_Sh")
{
}
...
@return -1 : 插入失败
*/
int MatchRule_Sh::InsertSingleRule_Withoutlock(rapidjson::Value& docValue, bool bUpdateToDb)
{
	...
	/*
	string sDebug;
	Tgw_StringUtil::GetJsonString(docValue, sDebug);
	BOOST_LOG_SEV(m_scl, trivial::info) << ftag << sDebug;
	*/

	MAP_SH_RULES::iterator itRule = m_mapShRules.find(ui64RuleId);

	if (m_mapShRules.end() == itRule)
	{
		// 未找到
		std::shared_ptr<struct MATCH_RULE_SH> ptr(new struct MATCH_RULE_SH());
		...
		{
			BOOST_LOG_SEV(m_scl, trivial::error) << ftag << "failed to SetRuleConfirm, ruleid=" << strRuleId;
		}

		if (docValue.HasMember(simutgw::client::cstrKey_rule_match)
			&& docValue[simutgw::client::cstrKey_rule_match].IsArray())
		{
			ptr->SetRuleMatch(docValue[simutgw::client::cstrKey_rule_match]);
		}
		else
		{
			BOOST_LOG_SEV(m_scl, trivial::error) << ftag << "failed to SetRuleMatch, ruleid=" << strRuleId;
		}

		m_mapShRules.insert(std::pair<uint64_t, std::shared_ptr<struct MATCH_RULE_SH>>(ui64RuleId, ptr));
	}
	else
	{
		// 找到
...
@return -1 : 更新失败
*/
int MatchRule_Sh::UpdateSingleRuleToDb(const std::string& strRuleId, const rapidjson::Value& in_docJudgeCond, rapidjson::Value& docValue)
{
	...
	try{
		...
		strUpdateSql += simutgw::g_DBTable_match_rule_sh;
		strUpdateSql += "(`idmatch_rule_sh`,`rule_name`,`judge_cond`,`rule_check`,"
			"`rule_confirm`,`rule_match`,`rule_wth`,`rule_settlement`,`is_delete`,"
			"`changer`,`change_time`) VALUES ('";

		// `idmatch_rule_sh`,`rule_name`,`regex_stock`,`regex_owflag`,`rule_check`,
		...
		if (!docValue.HasMember(simutgw::client::cstrKey_rule_check))
		{
			strUpdateSql += ",NULL";
		}
		else
...
@return -1 : 插入失败
*/
int MatchRule_Sh::InsertRule_Withoutlock(rapidjson::Value& docValue, bool bUpdateToDb)
{
...
@return -1 : 插入失败
*/
int MatchRule_Sh::InsertRule(rapidjson::Value& docValue, bool bUpdateToDb)
{
...
@return -1 : 获取失败
*/
int MatchRule_Sh::ReloadFromDb(void)
{
	...
	try
	{
		...
		// 查询用户信息及余额
		strQueryString = "SELECT `idmatch_rule_sh`,`judge_cond`,`rule_check`,`rule_confirm`,"
			"`rule_match`,`rule_wth`,`rule_settlement`,unix_timestamp(`change_time`) AS RULE_CHANGE_TIME FROM ";
		strQueryString += simutgw::g_DBTable_match_rule_sh;
		...
		if (1 == iRes)
		{
			...
			{
				rapidjson::Value jsonRuleElem(rapidjson::Type::kObjectType);

				// `idmatch_rule_sh`,
				strQueryValue = mapRowData["idmatch_rule_sh"].strValue;
				if (mapRowData["idmatch_rule_sh"].bIsNull || strQueryValue.empty())
				{
					BOOST_LOG_SEV(m_scl, trivial::error) << ftag << "invalid idmatch_rule_sh";
					iReturn = -1;
					break;
				}
				...
				if (!strQueryValue.empty())
				{
					...
					if (docData_judge_cond.Parse<0>(strQueryValue.c_str()).HasParseError() || docData_judge_cond.IsNull())
					{
						...
						BOOST_LOG_SEV(m_scl, trivial::error) << ftag << "Parse raw string to Json failed,string=[" << strQueryValue
							<< "] idmatch_rule_sh=" << mapRowData["idmatch_rule_sh"].strValue;
						return -1;
					}
		...
		/*
		string sDebug;
		Tgw_StringUtil::GetJsonString(docRuleArray, sDebug);
		BOOST_LOG_SEV(m_scl, trivial::info) << ftag << sDebug;
		*/
...
@return -1 : 获取失败
*/
int MatchRule_Sh::GetRule(const rapidjson::Value& in_docDeclareOrder,
	...
	uint64_t& out_ruleId, std::shared_ptr<struct MATCH_RULE_SH>& out_rule)
{
	...
	MAP_SH_RULES::iterator itRule = m_mapShRules.begin();
	for (; m_mapShRules.end() != itRule; ++itRule)
	{
		...
		// iterator
		std::map<std::string, std::regex>::const_iterator citRegex;
		for (citRegex = itRule->second->mapRegexs.cbegin();
			itRule->second->mapRegexs.cend() != citRegex;
			++citRegex)
		{
			if (!in_docDeclareOrder.HasMember(citRegex->first.c_str()))
			{
				// 订单中无改字段
				break;
			}

			string strOrdValue = in_docDeclareOrder[citRegex->first.c_str()].GetString();
...
*/
void MatchRule_Sh::CompareRuleTime_ToLocalRule(const std::map<uint64_t, uint64_t>& in_mapRuleTime, std::vector<uint64_t>& out_vctNeedFetchRule)
{
	...
	MAP_SH_RULES::iterator itLocalRule;
	for (citWebRuleTime = in_mapRuleTime.begin(); in_mapRuleTime.end() != citWebRuleTime; ++citWebRuleTime)
	{
		itLocalRule = m_mapShRules.find(citWebRuleTime->first);
		if (m_mapShRules.end() == itLocalRule)
		{
			// 未在本地找到，需从Web请求
