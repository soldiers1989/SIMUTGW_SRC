#ifndef __LNX_TCP_HANDLER_SIMUTGW_SERVER_H__
#define __LNX_TCP_HANDLER_SIMUTGW_SERVER_H__
...
class LnxTcpHandler_Simutgw_Server : public EpollServer_EventHandler
{
protected:
	...
	// 父业务类对象
	SimutgwTcpServer* m_pServer;

	HandlerSocketMsg m_handlerMsg;

public:
	explicit LnxTcpHandler_Simutgw_Server(SimutgwTcpServer* pServer);
	virtual ~LnxTcpHandler_Simutgw_Server(void);
...
private:
	// 禁止使用默认构造函数
	LnxTcpHandler_Simutgw_Server(void);
};
