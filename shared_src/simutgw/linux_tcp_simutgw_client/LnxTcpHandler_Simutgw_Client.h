#ifndef __LNX_TCP_HANDLER_SIMUTGW_CLIENT_H__
#define __LNX_TCP_HANDLER_SIMUTGW_CLIENT_H__

#include <vector>
#include <memory.h>

#include "util/EzLog.h"
#include "linux_epoll_client/EpollClient_EventHandler.h"
#include "socket_conn_manage_base/HandlerSocketMsg.h"
#include "linux_epoll_server/Epoll_ConnectionManager.h"

class Clienter;

class LnxTcpHandler_Simutgw_Client : public EpollClient_EventHandler
{
protected:
	static string m_strPing;
	static string m_strPong;

	src::severity_channel_logger<trivial::severity_level, std::string> m_scl;

	// 父业务类对象
	Clienter* m_pClienter;

	HandlerSocketMsg m_handlerMsg;

public:
	explicit LnxTcpHandler_Simutgw_Client(Clienter* pclienter);
	virtual ~LnxTcpHandler_Simutgw_Client(void);
...
private:
	// 禁止使用默认构造函数
	LnxTcpHandler_Simutgw_Client(void);
};
